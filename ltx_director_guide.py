...
class LTXDirectorGuide(LTXVAddGuide):
    ...
    def define_schema(cls):
        return io.Schema(
            node_id="SF-LTXDirectorGuide",
            display_name="SF-LTX 导演台引导",
    ...
    def execute(cls, positive, negative, vae, latent, guide_data, scale_by=1.0, upscale_method="bicubic") -> io.NodeOutput:
        ...
        for idx, img_tensor in enumerate(images):
            ...
            if t.shape[-2:] != (latent_height, latent_width):
                B, C, F, H, W = t.shape
                t_4d = t.permute(0, 2, 1, 3, 4).reshape(B * F, C, H, W)
                t_resized_4d = comfy.utils.common_upscale(
                    t_4d,
                    latent_width,
                    latent_height,
                    upscale_method,
                ...
                t = t_resized_4d.reshape(B, F, C, latent_height, latent_width).permute(0, 2, 1, 3, 4)
