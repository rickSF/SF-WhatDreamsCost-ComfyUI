...
// Global registry to track all SF-LTXKeyframer nodes across all subgraphs
...
// Finds all other SF-LTXKeyframer nodes globally and mirrors the value to them
function syncWidgetAcrossNodes(sourceNode, widgetName, value) {
    ...
    for (const targetNode of window._CSLTXKeyframerGlobalNodes) {
        // Target all OTHER SF-LTXKeyframer nodes by direct object reference
...
app.registerExtension({
    name: "Comfy.SF-LTXKeyframer.DynamicInputs",
    async nodeCreated(node) {
        if (node.comfyClass !== "SF-LTXKeyframer") return;
        ...
        // Helper: read image count from a connected SF-MultiImageLoader node
        function readSourceImageCount(self) {
            ...
            function traceUpstream(graph, linkId, visited = new Set()) {
                ...
                if (originNode.comfyClass === "SF-MultiImageLoader") {
                    return originNode;
                }
                ...
                if (typeof originNode.getInnerNode === "function") {
                    try {
                        ...
                        if (innerNode && innerNode.comfyClass === "SF-MultiImageLoader") {
                            return innerNode;
                        }
                    } catch (e) {
            ...
            // If there is EXACTLY ONE SF-MultiImageLoader in the workspace, safely assume that's the one.
            let multiImageLoaders = [];
            function findAllLoaders(nodes) {
                if (!nodes) return;
                for (let n of nodes) {
                    if (n.comfyClass === "SF-MultiImageLoader") {
