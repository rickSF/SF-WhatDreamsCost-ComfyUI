...
// Global registry to track all SF-LTXSequencer nodes across all subgraphs
...
app.registerExtension({
    name: "Comfy.SF-LTXSequencer.DynamicInputs",
    async nodeCreated(node) {
        if (node.comfyClass !== "SF-LTXSequencer") return;
        ...
        function readSourceImageCount(self) {
            ...
            function traceUpstream(graph, linkId, visited = new Set()) {
                ...
                if (originNode.comfyClass === "SF-MultiImageLoader") return originNode;
                ...
                if (typeof originNode.getInnerNode === "function") {
                    try {
                        ...
                        if (innerNode?.comfyClass === "SF-MultiImageLoader") return innerNode;
                    } catch (e) {}
                }
                return null;
            }
            ...
            function findAllLoaders(nodes) {
                if (!nodes) return;
                for (let n of nodes) {
                    if (n.comfyClass === "SF-MultiImageLoader") multiImageLoaders.push(n);
